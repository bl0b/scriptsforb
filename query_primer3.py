...
print "This script queries http://frodo.wi.mit.edu/primer3/. Use with all due respect."
...
# as of 6 may 2011
post_url = "http://frodo.wi.mit.edu/cgi-bin/primer3-web-cgi-bin-0.4.0/primer3_results.cgi"
state = {
...
def init_query():
    ...
    while True:
        ...
        if not data:
            break
        buf += data
    m = re.findall('select name="([^"]*)"()', buf) + re.findall('name="([^"]*)" value="([^"]*)"', buf) + re.findall('textarea name="([^"]*)"()', buf)
    print m
    state = dict(m)
    post_url = host + re.compile('form action="([^"]*)"').search(buf).groups()[0]
    print state
...
def init_args(args):
    ...
    for i in xrange(1, len(args)):
        if args[i] in ['-f', '--fast']:
            ...
            i += 1
            behaviour["output"] = open(args[i])
        elif args[i] in ['-h', '--help']:
            print "Query Primer3"
            print "by D. Leroux damien.leroux@gmail.com"
            print
            print "Usage: %s [-f,--fast] [-o,--output filename] [configuration_key=value...]"%args[0]
            ...
            key = key.upper()
            if ' ' not in key and state.has_key(key):
                new_state[key] = value
            else:
